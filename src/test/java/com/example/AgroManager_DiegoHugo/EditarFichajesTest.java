...
@SpringBootTest
@Transactional
public class EditarFichajesTest {
    ...
    @Test
    void editarFichaje_actualizaInicioFinEstado() {
        ...
        // Editar fichaje (típico caso gerente)
        Instant nuevoInicio = fich.getInicio().minusSeconds(3600);
        Instant nuevoFin = Instant.now();

        fich.setInicio(nuevoInicio);
        fich.setFin(nuevoFin);
        fich.setEstado("CORREGIDO");
        fichajeRepository.save(fich);

        Fichaje rec = fichajeRepository.findById(fich.getId()).orElseThrow();
        assertEquals("CORREGIDO", rec.getEstado());
        assertEquals(nuevoInicio, rec.getInicio());
        assertEquals(nuevoFin, rec.getFin());
    }
}
